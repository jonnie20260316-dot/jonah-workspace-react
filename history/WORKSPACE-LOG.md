...
## 2026-04-04 | AI Chat Block (Claude.ai + ChatGPT Webview) — New block type embedding claude.ai and chatgpt.com via Electron webview; single block with Claude/ChatGPT tab switcher; `partition="persist:aichat"` for shared login session; header-stripping in session handlers to bypass X-Frame-Options/CSP frame-ancestors; tab state persisted via GLOBAL_KEY_PREFIXES; bilingual UI with graceful non-Electron fallback; 6 files changed, +143 lines; build ✓ 1796 modules; fa46698. See history/recaps/2026-04-04-ai-chat-block.md
