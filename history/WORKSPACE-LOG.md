...
## 2026-04-07 (wrap-up) | Sticky Daily Scope + AI Chat Stability + Text Centering + Screenshot Block + Error Audit — (1) Sticky notes converted to daily-scoped storage with one-time migration (migrateStickyToDaily gated by flag in GLOBAL_KEYS), StickyHistoryPanel modal scans all dates, DatePeekModal sticky tab for viewing past-day notes, sidebar date history collapsible; (2) AI Chat webview dual-render pattern (both Claude.ai + ChatGPT always in DOM, inactive hidden with visibility:hidden), keepMounted:true flag, fixes reset-on-tab-switch; (3) Text vertical alignment (top/middle/bottom) via verticalAlign SurfaceElement field + flexbox wrapper in SurfaceBackground + UI controls in TextPropertiesPanel; (4) New Screenshot block type (paste + upload, base64 storage, 5MB limit, caption metadata); (5) Archived blocks sidebar section with restore button; (6) Error audit documentation (58 bugs across 7 categories, 40+ prevention rules, 2 new rules). 17 files, +912 net lines. Build ✓ 1820 modules, 0 TypeScript errors. New rules: WEBVIEW-PERSIST-1, STORAGE-MIGRATION-1. See history/recaps/2026-04-07-sticky-ai-chat-text-centering-screenshot-audit.md.

## 2026-04-07 (evening) | YouTube Studio Determinism — Stream health for `ready` state broadcasts; auto-refresh after RTMP starts (8s+18s delays); RTMP cleanup on unmount (JW-28); clear stale error on transition retry. 1 file, +42 net lines. Build ✓ 1818 modules, 0 TypeScript errors. See history/recaps/2026-04-07-session-recap.md

## 2026-04-07 (wrap-up) | Note Recovery + Video Aspect + CSS + Text Resize — (1) Sticky note recovery: GearMenu button "♻ 從備份恢復便利貼" reads backup JSON, targeted restore of :block-global:sticky-* keys only, no side effects on layout/viewport. (2) Video aspect ratio: replaced height:"320px" with width:"100%" + aspectRatio:"16/9" for 16:9 letterbox on any block size. (3) Block collapse CSS: .block-body--hidden flex:0 0 0px → height:0 (more reliable for video/canvas dimension reporting). (4) Text element resize: converted 4 corner dots from pointerEvents:none to interactive handles using existing onHandlePointerDown/Move/Up, mapped to "nw"|"ne"|"se"|"sw" corners. 4 files, +98 lines; Build ✓ 1818 modules, 0 TypeScript errors. See history/recaps/2026-04-07-session-recap.md

## 2026-04-07 (later) | Editable Block Titles + Sticky Note Refactor + Text Formatting UI — (1) Editable block titles: new `editableTitle` config option in BlockRegistry; click header to edit, Escape/blur to save; `block.label` field for custom titles; delete button (X) with confirmation; (2) StickyBlock complete safety refactor: ref-based contentEditable (prevents React unmount wipes), isFocusedRef gate (prevents mid-typing rewrites), blur-only sync (keystroke saves on blur not onInput), data guards (isConnected check prevents unmount blank save), title support from block.label; (3) TextPropertiesPanel floating UI at bottom + text dragging in SurfaceForeground (startTextDrag, position updates); text formatting controls stubbed (color/font/size not yet wired); keepMounted block state + CSS .block-body--hidden (visibility:hidden not display:none to preserve <video>/<audio>/<canvas>); 11 files, +541 lines, -35 lines; Build ✓ 1818 modules, 0 TypeScript errors. See history/recaps/2026-04-07-editable-blocks-sticky-text.md
